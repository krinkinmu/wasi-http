...
WASM_EXPORT(void, proxy_abi_version_0_2_1, ()) {}
...
WASM_EXPORT(uint32_t, proxy_on_vm_start, (uint32_t root_context_id,
...
WASM_EXPORT(uint32_t, proxy_on_configure, (uint32_t root_context_id,
                                           uint32_t configuration_size)) {
...
WASM_EXPORT(void, proxy_on_context_create, (uint32_t context_id,
...
WASM_EXPORT(FilterStatus, proxy_on_new_connection, (uint32_t context_id)) {
  return getContext(context_id)->onNewConnection();
}

WASM_EXPORT(FilterStatus, proxy_on_downstream_data, (uint32_t context_id,
...
WASM_EXPORT(FilterStatus, proxy_on_upstream_data, (uint32_t context_id,
                                                   uint32_t data_length,
                                                   uint32_t end_of_stream)) {
  return getContext(context_id)
...
WASM_EXPORT(void, proxy_on_downstream_connection_close, (uint32_t context_id,
                                                         uint32_t close_type)) {
  return getContext(context_id)->onDownstreamConnectionClose(static_cast<CloseType>(close_type));
}

WASM_EXPORT(void, proxy_on_upstream_connection_close, (uint32_t context_id,
                                                       uint32_t close_type)) {
  return getContext(context_id)->onUpstreamConnectionClose(static_cast<CloseType>(close_type));
}

WASM_EXPORT(FilterHeadersStatus,
            proxy_on_request_headers,
            (uint32_t context_id, uint32_t headers, uint32_t end_of_stream)) {
  return getContext(context_id)->onRequestHeaders(headers, end_of_stream != 0);
}

WASM_EXPORT(FilterMetadataStatus, proxy_on_request_metadata, (uint32_t context_id,
                                                              uint32_t elements)) {
...
WASM_EXPORT(FilterTrailersStatus, proxy_on_request_trailers, (uint32_t context_id,
                                                              uint32_t trailers)) {
...
WASM_EXPORT(FilterMetadataStatus, proxy_on_response_metadata, (uint32_t context_id,
                                                               uint32_t elements)) {
...
WASM_EXPORT(FilterTrailersStatus, proxy_on_response_trailers, (uint32_t context_id,
                                                               uint32_t trailers)) {
...
WASM_EXPORT(void, proxy_on_http_call_response, (uint32_t context_id,
...
WASM_EXPORT(void,
            ...
            (uint32_t context_id, uint32_t token, uint32_t trailers)) {
  getRootContext(context_id)->onGrpcReceiveTrailingMetadata(token, trailers);
}

WASM_EXPORT(void, proxy_on_grpc_receive, (uint32_t context_id, uint32_t token,
                                          uint32_t response_size)) {
  getRootContext(context_id)->onGrpcReceive(token, static_cast<size_t>(response_size));
}

WASM_EXPORT(void, proxy_on_grpc_close, (uint32_t context_id, uint32_t token,
                                        uint32_t status_code)) {
  getRootContext(context_id)->onGrpcClose(token, static_cast<GrpcStatus>(status_code));
}

WASM_EXPORT(void, proxy_on_queue_ready, (uint32_t context_id, uint32_t token)) {
  getRootContext(context_id)->onQueueReady(token);
}

WASM_EXPORT(void,
            proxy_on_foreign_function,
            (uint32_t context_id, uint32_t foreign_function_id, uint32_t data_size)) {
